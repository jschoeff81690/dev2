...
<?php	
		...
		foreach($this->invoice->services as $ser)
		{
			...
			echo '<td style="text-align:center;">$'.$this->format_money($ser['cost'],TRUE)."</td>";
			echo "</tr>\n";
			$c++;
		}
		echo '<tr class="even"><td></td><td></td><td></td></tr>';
		echo '<tr><td></td><td style="text-align:right;">Subtotal</td><td style="text-align:center;">$'.$this->format_money($this->invoice->subtotal).'</td></tr><br />';
		echo '<tr><td></td><td style="text-align:right;">Tax</td><td style="text-align:center;">$'.$this->invoice->tax.'</td></tr><br />';
		echo '<tr><td></td><td style="text-align:right;border-style:none;border-size:0px;">Total</td><td style="text-align:center;">$'.$this->format_money($this->invoice->total,TRUE). "</td></tr><br />\n";
		echo "</table>\n";
		echo '<br /><a href="/dev/invoices/'.$this->invoice->id.'/pay"> Pay this invoice now </a>';
?>
</div>
